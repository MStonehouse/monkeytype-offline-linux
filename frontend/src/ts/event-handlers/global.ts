...
function isOfflineNetworkError(value: unknown): boolean {
  const text = String(
...
window.onerror = function (message, url, line, column, error): void {
  if (isDevEnvironment()) {
    if (!isOfflineNetworkError(error ?? message)) {
      showErrorNotification(error?.message ?? String(message ?? "Undefined message"), {
...
window.onunhandledrejection = function (e): void {
  if (isDevEnvironment()) {
    ...
    if (!isOfflineNetworkError(reason)) {
      showErrorNotification(
        reason instanceof Error ? reason.message : String(reason ?? "Undefined message"),
        {
