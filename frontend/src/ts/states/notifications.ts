...
function isExpectedOfflineNotification(
  message: string,
  ...
  if (options.response !== undefined) {
    parts.push(String(options.response.status));
    parts.push(String(options.response.body?.message ?? ""));
  }

  const text = parts.join(" ").toLowerCase();
  return [
    "external network access disabled in offline edition",
    "failed to fetch",
    "fetch failed",
    "networkerror",
    "network error",
...
export function addNotificationWithLevel(
  ...
  // Offline edition: expected backend/network failures are normal and should
  // not become red startup popups. Keep all other notifications unchanged.
  if (level === "error" && isExpectedOfflineNotification(message, options)) {
    console.info("Offline edition: suppressed expected network notification", {
      message,
